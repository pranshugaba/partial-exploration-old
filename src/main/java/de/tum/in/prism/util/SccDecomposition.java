...
public final class SccDecomposition {
  ...
  private final Int2ObjectFunction<IntIterable> successorFunction;
  private int index = 0;

  private SccDecomposition(Int2ObjectFunction<IntIterable> successorFunction,
      boolean includeTransient) {
    this.successorFunction = successorFunction;
    this.includeTransient = includeTransient;
  }

  public static List<NatBitSet> computeSccs(Int2ObjectFunction<IntIterable> function,
      IntSet initialStates, boolean includeTransient) {
  ...
  public static boolean isTransient(Int2ObjectFunction<IntIterable> successorFunction,
      NatBitSet scc) {
    if (scc.size() > 1) {
      return false;
    }
  ...
  private TarjanState create(int node) {
    ...
    IntIterator successorIterator = successorFunction.apply(node).iterator();
  ...
  private void run(int initial) {
    ...
    while (true) {
      ...
      IntIterator successorIterator = state.successorIterator;
  ...
  private static final class TarjanState {
    final int node;
    final int nodeIndex;
    final IntIterator successorIterator;
    int lowLink;

    TarjanState(int node, int nodeIndex, IntIterator successorIterator) {
      this.node = node;
