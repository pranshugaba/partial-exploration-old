...
public abstract class AbstractExplorer<M extends Model> implements Explorer<M> {
  ...
  protected AbstractExplorer(ModelGenerator generator, M model) {
  ...
  protected void initialize() throws PrismException {
    ...
    for (State initialState : generator.getInitialStates()) {
      int stateNumber = getStateNumber(initialState);
  ...
  @Override
  public State exploreState(int stateNumber) throws PrismException {
    assert stateMap.check(stateNumber) && !isExploredState(stateNumber);
  ...
  protected final int getStateNumber(State state) {
    assert state != null;
